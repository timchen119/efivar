...
extern ssize_t _make_hd_dn_udev(uint8_t *buf, ssize_t size, const char *devpath,
			   uint32_t partition, uint32_t options)
	__attribute__((__visibility__ ("hidden")));	
...
#define make_hd_dn_udev(buf, size, off, devpath, partition, option) \
	_make_hd_dn_udev(((buf)+(off)), ((size)?((size)-(off)):0), (devpath),\
		    (partition), (options))
